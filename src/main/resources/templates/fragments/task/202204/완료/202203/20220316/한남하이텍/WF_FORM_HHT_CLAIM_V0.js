...
function COST_CAL() {
    ...
    for (i = 3 ; i < tblobj.rows.length; i++) {
        var cost = $(tblobj.rows[i]).find("input[name=itmQnt]").val() * $(tblobj.rows[i]).find("input[name=itmPrice]").val() //수량 * 단가 = 금액
        
        // 값을 입력하지 않았을 경우 계산방지
    	if(isEmptyStr($(tblobj.rows[i]).find("input[name=itmQnt]").val()) && isEmptyStr($(tblobj.rows[i]).find("input[name=itmPrice]").val())) {
    		bEmpty = true;
    	}
        
        if(!bEmpty) {
        	$(tblobj.rows[i]).find("input[name=itmAmt]").val(cost); //수량 * 단가 = 금액
        }
        // 부가세 별도 표시시 추가해야됨
        // $(tblobj.rows[i]).find("input[name=TAX]").val(parseFloat(cost * 0.1));
    	
    	// 비어있는 필드는 NaN으로 간주하기 때문에 number체크 해줘야됨
    	if(!isNaN(parseFloat($(tblobj.rows[i]).find("input[name=itmAmt]").val()))) {
    		cost_sum += parseFloat($(tblobj.rows[i]).find("input[name=itmAmt]").val());
    	}
    }
