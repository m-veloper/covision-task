...
function postRenderingForTemplate() {
	...
	var m_objXML = getInfo("FileInfos");      //JSON 가져오기
	 ...
	 //$("#headname").hide();
	 $("#tblFormSubject").hide();
	 $(".tableStyle.linePlus.mt10.fsbTop").hide();
	 $('#btPreView').hide();
    ...
    if($("#TEL").val() == ""){
    	 $("#TEL").val(Common.GetObjectInfo("UR", getInfo("AppInfo.usid"), "Mobile").Mobile  );		// 02-6965-3100
    }
    ...
    if (getInfo("Request.templatemode") == "Read") {
       ...
       //******************** 양식 파일 내 첨부파일 함수 정리 시작 ***************************************************************************************************************************************/
        ...
        if (formJson.Request.mode == "DRAFT" || formJson.Request.mode == "TEMPSAVE") {
        	...
        	if($("#Subject").val()!="" && $("#titleHeader").val()==""){
        		$("#titleHeader").val($("#Subject").val());
        		
        	}
        	
        	$("#zipNo").val(Common.getBaseConfig("APV_EXTERNAL_ADD"));	
        	$("#webAddress").val("www.unesco-whipic.org");	
            ...
            if($("#managerEmail").val() == ""){
            	 document.getElementById("managerEmail").value = getInfo("AppInfo.ussip");
              }
            
            if($("#phone").val() == ""){
           	 $("#phone").val(Common.GetObjectInfo("UR", getInfo("AppInfo.usid"), "Mobile").Mobile  );		// 02-6965-3100
           }
            
            getInfo("AppInfo.ussip")  
