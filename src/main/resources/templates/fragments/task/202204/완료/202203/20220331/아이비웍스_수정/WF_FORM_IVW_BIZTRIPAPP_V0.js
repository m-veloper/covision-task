...
function Requester_CallBack(pStrItemInfo) {
   ...
   console.log(" I_User :: " + JSON.stringify(I_User));
   ...
   if(I_User != undefined){
    if (getInfo("AppInfo.usid") != I_User.AN) {
        ...
        document.getElementById("DEPUTY_NAME").value = CFN_GetDicInfo(I_User.DN); //성명
        
        
        document.getElementById("ExGroupName").value = CFN_GetDicInfo(I_User.RGNM); //소속
        document.getElementById("PO").value = CFN_GetDicInfo(I_User.PO.split('&')[1]);// 직위
    }
   }
}
