...
function makeBodyContext() {
	...
	$$(bodyContextObj["BodyContext"]).append(getDoubleMultiRowFields("ItemTbl", "subTbl", "rField"));
...
function delRow(){
	...
	len = $("table[name=subTbl]").length;
	len2 = $("input[name=CardUseComment]").length;
	
	if(len > 1){
		$("table[name=subTbl]").eq(len-1).parent().remove()
	}
	if(len2 > 1){
		$("input[name=CardUseComment]").eq(len2-1).closest("tr").remove();
	}
