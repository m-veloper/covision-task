...
function postRenderingForTemplate() {
    ...
    if (getInfo("Request.templatemode") == "Read") {
        ...
        if (formJson.Request.mode == "TEMPSAVE") {
        	//임시저장시 selectBox setting을 위한 함수 실행	
        	  settingSelect();
...
function initDivSelect(obj) {
    ...
    if($(obj).val() == "0" ){
    	...
    	$(obj).parent().parent().find("[name=REQ_Div7]").css("display", "none");
    	$(obj).parent().parent().find("[name=REQ_Div8]").css("display", "none"); 
    	...
    	$(obj).parent().parent().find("[name=REQ_Div5]").css("display", "none");
    	$(obj).parent().parent().find("[name=REQ_Div6]").css("display", "none"); 
    	$(obj).parent().parent().find("[name=REQ_Div7]").css("display", "block");
    	$(obj).parent().parent().find("[name=REQ_Div8]").css("display", "none"); 
    	$(obj).parent().parent().find("[name=REQ_Div2]").val(0);
    	$(obj).parent().parent().find("[name=REQ_Div3]").val(0);
    	$(obj).parent().parent().find("[name=REQ_Div4]").val(0);
    	...
    	$(obj).parent().parent().find("[name=REQ_Div6]").css("display", "none"); 
    	$(obj).parent().parent().find("[name=REQ_Div7]").css("display", "none");
    	$(obj).parent().parent().find("[name=REQ_Div8]").css("display", "block"); 
    	$(obj).parent().parent().find("[name=REQ_Div2]").val(0);
    	$(obj).parent().parent().find("[name=REQ_Div3]").val(0);
    	$(obj).parent().parent().find("[name=REQ_Div4]").val(0);
    	$(obj).parent().parent().find("[name=REQ_Div5]").val(0);
...
function initDivInput(obj, id){
	if(id =="2"){
		...
		$(obj).parent().parent().find("[name=REQ_DivInputName]").val($(obj).parent().find("[name=REQ_Div7] option:checked").text()); 
	}else if(id =="8"){
		$(obj).parent().parent().find("[name=REQ_DivInputName]").val($(obj).parent().find("[name=REQ_Div8] option:checked").text()); 
	}
    	...
    	if($(tblobj.rows[i]).find("input[name=REQ_PRICE]").val() != undefined) {
    		if(isEmptyStr($(tblobj.rows[i]).find("input[name=REQ_PRICE]").val())) {
    			...
    			if(cost_Max <= parseFloat($(tblobj.rows[i]).find("input[name=REQ_PRICE]").val()) ){
				    ...
				    }else if($(tblobj.rows[i]).find("[name=REQ_Div1]").val() ==6){
				    	sREQ_Div2 =$(tblobj.rows[i]).find("[name=REQ_Div7] option:checked").text(); //항목2
				    }else if($(tblobj.rows[i]).find("[name=REQ_Div1]").val() ==7){
				    	sREQ_Div2 =$(tblobj.rows[i]).find("[name=REQ_Div8] option:checked").text(); //항목2
				    }	    
...
function settingSelect() {
    ...
    for (i = 3 ; i < tblobj.rows.length; i++) { 
    	...
    	if($(tblobj.rows[i]).find("[name=REQ_Div1]").val() ==1){
			...
			$(tblobj.rows[i]).find("[name=REQ_Div7]").css("display", "block");
	    }else if($(tblobj.rows[i]).find("[name=REQ_Div1]").val() ==7){
			$(tblobj.rows[i]).find("[name=REQ_Div8]").css("display", "block"); 
	    }	
    }
