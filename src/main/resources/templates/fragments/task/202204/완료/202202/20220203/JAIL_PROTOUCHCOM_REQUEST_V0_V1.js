...
function vauelChange(obj, stringId){
	 var contTotal = document.getElementsByName("ContTotal");
	 var tblobj = document.getElementById("MULTI_TABLE03");
	 console.log(tblobj.length);
...
function COST_CAL() {
    var tblobj = document.getElementById("TB_OBJECT"); //멀티로우 TABLE ID
    var cost_sum = 0;
    

	 $("[name=ContTotal]").each(function () {
		 cost_sum += parseFloat(($(this).val() == "" ? 0 : $(this).val()));
	    });

	 
    $("#Matters").val(cost_sum);
	$("#Hang").val(cost_sum);
	$("#Ho").val(cost_sum);
}
