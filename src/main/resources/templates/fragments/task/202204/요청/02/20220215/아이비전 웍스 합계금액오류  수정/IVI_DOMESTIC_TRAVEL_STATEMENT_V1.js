...
function calDATE() {
    ...
    var SDATE = $("#SStart_Date").val().split('-');
    var EDATE = $("#EEnd_Date").val().split('-');
    
		if($("#SStart_Date").val()!="" && $("#EEnd_Date").val()!=""){
	        ...
	        $("#TTotal_Day").val((tmpday+1));
		    EASY.triggerFormChanged();
		    if (tmpday < 0) {
		        alert("이전 일보다 전 입니다. 확인하여 주십시오.");
		        $("input[id=EEnd_Date]").val("");
		        $("input[id=TTotal_Day]").val("");
		    }
		}
}

function calSSDATEEDATE(obj) {

	var tmpObj = $(obj).closest("tr");	
	
    var SDATE = $(tmpObj).find("input[name=Start_Date]").val().split('-');
    var EDATE = $(tmpObj).find("input[name=End_Date]").val().split('-');
  
	if($(tmpObj).find("input[name=Start_Date]").val()!="" && $(tmpObj).find("input[name=End_Date]").val()!=""){
	    var SOBJDATE = new Date(parseInt(SDATE[0], 10), parseInt(SDATE[1], 10) - 1, parseInt(SDATE[2], 10));
	    var EOBJDATE = new Date(parseInt(EDATE[0], 10), parseInt(EDATE[1], 10) - 1, parseInt(EDATE[2], 10));
	    var tmpday = EOBJDATE - SOBJDATE;
        tmpday = parseInt(tmpday, 10) / (1000 * 3600 * 24);
        ...
        if(tmpday<0){
        	Common.Warning('옳바른 값을 입력해주세요.');
        }else{
        	$(tmpObj).find("input[name=Total_Day]").val((tmpday+1));
        }

	}
}

function calDaily(num){
	
	var trobj= $(num).closest("tr");

	var sum=0;
	
	$("input[name=Daily_Cost]").each(function(i,obj){
		
		var ttobj = $(obj).closest("tr");
		
		sum += toNum($(ttobj).find("input[name=Daily_Cost]"));
    });
	
	$("#Total_Daily").val(sum);
	
	EASY.formatter($("#Total_Daily"));
	EASY.formatter($("#Total_Cost"));
}
//콤마 붙은 문자 숫자로 변경
function toNum(obj){
