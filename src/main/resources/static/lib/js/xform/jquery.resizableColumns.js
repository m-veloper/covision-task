  ...
  **/
		value: function assignPercentageWidths() {
			...
			this.$tableHeaders.each(function (_, el) {
				var $el = $(el);
				headers.push( $el.outerWidth() == '' ? 0 : $el.outerWidth() );
				totalWidth += headers[_];
			});
			
			this.$tableHeaders.each(function (_, el) {
				if(headers[_] === 0) totalWidth / this.$tableHeaders.length ;
				_this2.setWidth($(el)[0], 100 * headers[_] / totalWidth);
			});
