...
var m_oRecList;
var sReceiveNo = "";
var g_BaseImgURL = Common.getBaseConfig("BackStorage") + "e-sign/ApprovalSign/BackStamp/";
var g_BaseFormURL = Common.getBaseConfig("BackStorage") + "e-sign/ApprovalForm/"; // 사용안함
...
function initialize() {
	...
	if(getInfo("Request.templatemode") == "Write"){
		...
		if(getInfo('ExtInfo.UseHWPEditYN') == "Y" && $("#writeTab li.on").length > 0){
			$("#AttFileInfoList").hide();
			$("#AttFileInfoList_Multi").show();
		} else {
			$("#AttFileInfoList").show();
			$("#AttFileInfoList_Multi").hide();
		}
	}
	...
	if(getInfo('ExtInfo.UseHWPEditYN') == "Y" && $("#writeTab li.on").length > 0){
		...
		$("#DocLinkInfoList_Multi").show();
	} else {
		$("#DocLinkInfoList").show();
		$("#DocLinkInfoList_Multi").hide();
	}
...
function initForm() {
    ...
    //EDMS 연결문서 관련
    try { G_displaySpnEDMSDocLinkInfo(); } catch (e) { }
    ...
    if (getInfo("Request.templatemode") == "Write") {
        ...
        switch (commonEditorType) {
            ...
            case editortypes['xfree']:
            	...
            	if (getInfo("BodyContext") != undefined && getInfo("BodyContext") != "{}") {
            		if (formJson.BodyContext.tbContentElement["#cdata-section"]) {
            			coviEditor.setBody("xfree", 'tbContentElement', formJson.BodyContext.tbContentElement["#cdata-section"].replace(/\\n/gi, ""));
            			...
            			coviEditor.setBody("xfree", 'tbContentElement', formJson.BodyContext.tbContentElement.replace(/\\n/gi, ""));
                    ...
                    if (getInfo("FormInfo.BodyDefault") != undefined) {
                        try {
                        	coviEditor.setBody("xfree", 'tbContentElement', Base64.b64_to_utf8(getInfo("FormInfo.BodyDefault")));
            ...
            case editortypes['webhwpctrl']:  
				try{
		            if (getInfo("BodyContext") != undefined && getInfo("BodyContext") != "{}") {
		            	if (formJson.BodyContext.tbContentElement["#cdata-section"]) {
		            		coviEditor.setBody("webhwpctrl", 'tbContentElement', formJson.BodyContext.tbContentElement["#cdata-section"].replace(/\\n/gi, ""));
		            	} else {
		            		coviEditor.setBody("webhwpctrl", 'tbContentElement', formJson.BodyContext.tbContentElement.replace(/\\n/gi, ""));
		            	}
		                try { G_displaySpnDocLinkInfo(); } catch (e) { }
		            } else {
		                if (getInfo("FormInfo.BodyDefault") != undefined) {
		                	coviEditor.setBody("webhwpctrl", 'tbContentElement', Base64.b64_to_utf8(getInfo("FormInfo.BodyDefault")));
		                }
		            }
				} catch (e) {
					setTimeout("setEditor()", 100);
...
function getChkRadio(radioNm) {
    if (eval(radioNm).checked) {
        var radioVal = makeNode(radioNm, eval(radioNm).value);
        return radioVal;
    }
}
    ...
    $(l_editor).find("input[data-type=dField], textarea[data-type=dField], checkbox[data-type=dField]", "radio[data-type=dField]").each(function (i, fld) {
        if ($(fld).attr("id") == "AttachFileInfo") {
        	...
        	if($(fld).val() == "@@" && getInfo("FormInstanceInfo."+$(fld).attr("id")) == "") {
        		...
        		if(getInfo("Request.templatemode") == "Write") {
	        		if (!($(fld).val() == "" && getInfo("FormInstanceInfo."+$(fld).attr("id")) == undefined)) { //$(fld).val() != getInfo("FormInstanceInfo."+$(fld).attr("id")) && 
	    	            var fldVal = $(fld).val();
	    	            $.extend(jsonObj, makeNode($(fld).attr("id"), fldVal));
	                }	
        		} else {
...
function OpenDocClass() {
    ...
    if (m_oFormMenu.gDocboxMenu == "T") {
        var sUrl = "";
        sUrl = "admin/goDocTreePop.do?domainID=" + getInfo("AppInfo.etid");
    ...
    if (szdoclinks != "") {
        ...
        for (var i = 0; i < adoclinks.length; i++) {
            ...
            if (bEdit) {
                ...
                szdoclinksinfo += "&ownerProcessId=" + (getInfo("ProcessInfo.ProcessID") == undefined ? "" : getInfo("ProcessInfo.ProcessID")); // 현문서 ProcessId [2021-01-22 add]
                
                if(typeof adoc[6] != "undefined" && adoc[6] != "undefined")
                	szdoclinksinfo += "&ExpAppID="+adoc[6]+"&taskID="; // 경비 팝업에서 필요한 파라미터 추가
...
function G_displaySpnDocLinkInfoMulti(idx) {//다안기안용 연결문서
    var szdoclinksinfo = "";
    var szdoclinks = "";
    try { szdoclinks = document.getElementsByName("MULTI_LINK_DOC")[idx].value; } catch (e) { }
    
    if (getInfo("Request.mode") == "DRAFT" || getInfo("Request.mode") == "TEMPSAVE" || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "SUBAPPROVAL") || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "SUBREDRAFT") || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "PCONSULT") || g_szEditable == true) {
        
    } else {
        if (szdoclinks == "") {
            try { document.getElementsByName("MULTI_LINK_DOC")[idx].value = szdoclinks; } catch (e) { }
        }
    }
    ...
    if (szdoclinks != "") {
        ...
        if (bEdit) {
            if (szdoclinksinfo != "" && (getInfo("Request.mode") == "DRAFT" || getInfo("Request.mode") == "TEMPSAVE" || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "SUBAPPROVAL")) || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "PCONSULT") || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "SUBREDRAFT") || g_szEditable == true) {
                szdoclinksinfo += "<div class='xbtn' style='float:right;' ><input type='button' class='AXButton' value='연결삭제' onclick='deletedocitem(true, "+idx+");'></div>";
            }
        }
    }
    if(document.getElementById("DocLinkInfo"+idx) != undefined) {
    	document.getElementById("DocLinkInfo"+idx).innerHTML = szdoclinksinfo;
    }
}
...
function G_displaySpnEDMSDocLinkInfo() {//수정본
    var szdoclinksinfo = "";
    var szdoclinks = "";
    if (getInfo("Request.mode") == "DRAFT" || getInfo("Request.mode") == "TEMPSAVE" || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "SUBAPPROVAL") || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "SUBREDRAFT") || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "PCONSULT") || g_szEditable == true) {
        try { szdoclinks = document.getElementById("EDMSDocLinks").value; } catch (e) { }
    } else {
        try { szdoclinks = document.getElementById("EDMSDocLinks").value; } catch (e) { }
        if (szdoclinks == "") {
            try { document.getElementById("EDMSDocLinks").value = szdoclinks; } catch (e) { }
        }
    }
    szdoclinks = szdoclinks.replace("undefined^", "");
    szdoclinks = szdoclinks.replace("undefined", "");
    if (szdoclinks != "") {
        ...
        for (var i = 0; i < adoclinks.length; i++) {
            ...
            if (bEdit) {
                if (getInfo("Request.mode") == "DRAFT" || getInfo("Request.mode") == "TEMPSAVE" || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "SUBAPPROVAL") || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "PCONSULT") || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "SUBREDRAFT") || g_szEditable == true) {
                    szdoclinksinfo += "<input type=checkbox id='chkDoc' name='_" + adoc[4] + "' value='" + adoc[4] + "' class='td_check' style='float:none;'>";
                }
                szdoclinksinfo += "<span class='td_txt' onmouseover='this.style.color=\"#2f71ba\";' onmouseout='this.style.color=\"#111111\";'  style='cursor:pointer;float:none;'  onclick=\"goEDMSBoardViewPopup('";
                szdoclinksinfo += adoc[0] + "','" +  adoc[1] + "','" + adoc[2]+ "','" +adoc[3]+ "','" +adoc[4] + "') \">" + adoc[5] + "</span><br />";
            } else {
                if (bDisplayOnly) {
                    szdoclinksinfo += adoc[5];
                } else {
                    szdoclinksinfo += "<span class='txt_gn11_blur' onmouseover='this.style.color=\"#2f71ba\";' onmouseout='this.style.color=\"#111111\";'  style='cursor:pointer;'  onclick=\"goEDMSBoardViewPopup('";
                    szdoclinksinfo += adoc[0] + "','" +  adoc[1] + "','" + adoc[2]+ "','" +adoc[3]+ "','" +adoc[4] + "') \">- " + adoc[5] + "</span><br />";
                }
            }
        }
        if (bEdit) {
            if (szdoclinksinfo != "" && (getInfo("Request.mode") == "DRAFT" || getInfo("Request.mode") == "TEMPSAVE" || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "SUBAPPROVAL")) || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "PCONSULT") || (getInfo("Request.loct") == "APPROVAL" && getInfo("Request.mode") == "SUBREDRAFT") || g_szEditable == true) {
            	szdoclinksinfo += "<div class='xbtn' style='float:right;' ><input type='button' class='AXButton' value='연결삭제' onclick='deleteEdmsDocItem();'></div>";
            }
        }
    }
    document.getElementById("EDMSDocLinkInfo").innerHTML = szdoclinksinfo;
}

function goEDMSBoardViewPopup(pBizSection, pMenuID, pVersion, pFolderID, pMessageID){
...
function InputDocLinks(szValue) {
    try {
    	if (getInfo('ExtInfo.UseHWPEditYN') == "Y" && $("#writeTab li.on").length > 0) {
    		var idx = Number($("#writeTab li.on").attr('idx'));
    		
    		if(document.getElementsByName("MULTI_LINK_DOC")[idx].value == "") {
    			document.getElementsByName("MULTI_LINK_DOC")[idx].value = szValue;
    			G_displaySpnDocLinkInfoMulti(idx);
    		} else {
	            adddocitem(szValue, true, idx);
    		}
    	} else {
...
function adddocitem(szAddDocLinks, isMulti, idx) {
    var adoclinks = [];
    if(isMulti == true && idx != undefined) {
        adoclinks = document.getElementsByName("MULTI_LINK_DOC")[idx].value.split("^^^");
    } else {
    	adoclinks = document.getElementById("DocLinks").value.split("^^^");
    }
    ...
    if(isMulti == true && idx != undefined) {
        document.getElementsByName("MULTI_LINK_DOC")[idx].value = szdoclinksinfo;
        G_displaySpnDocLinkInfoMulti(idx);
    } else {
...
function deletedocitem(isMulti, idx) {
    var adoclinks = [];
    var chkDoc = [];
    if(isMulti == true && idx != undefined) {
    	adoclinks = document.getElementsByName("MULTI_LINK_DOC")[idx].value.split("^^^");
        chkDoc = $("#DocLinkInfo"+idx).find("input[id='chkDoc']");
    } else{
    	adoclinks = document.getElementById("DocLinks").value.split("^^^");
    	chkDoc = $("#DocLinkInfo").find("input[id='chkDoc']");
    }
    var szdoclinksinfo = "";
    var tmp = "";
    if (chkDoc.length > 0) {
...
function deleteEdmsDocItem(){
    var adoclinks = [];
    var chkDoc = [];
	adoclinks = document.getElementById("EDMSDocLinks").value.split("^^^");
	chkDoc = $("#EDMSDocLinkInfo").find("input[id='chkDoc']");
    ...
    document.getElementById("EDMSDocLinks").value = szdoclinksinfo;
    G_displaySpnEDMSDocLinkInfo();    
}
...
function event_attchSync(dataresponseXML) {
    ...
    if (errorNode.length > 0) {
        ...
        if (editortype.is('xfree') || (editortype.is('tagfreenxfree') && window.ActiveXObject === undefined)) {
            ...
            var g_szURL = Common.getBaseConfig("BackStorage") + "e-sign/Approval/IMAGEATTACH"; //"http://" + document.location.host + 
