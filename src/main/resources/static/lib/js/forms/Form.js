...
Common.getDicList(["DOC_LEVEL_10", "DOC_LEVEL_20","DOC_LEVEL_30","lbl_apv_year_1","lbl_apv_year_3","lbl_apv_year_5","lbl_apv_year_7","lbl_apv_year_10"
                   ...
                   , "lbl_apv_setschema_Circulation", "MsgApprovalState_ApprovalConsulted", "lbl_apv_OTransPV", "msg_apv_autoChkAgree", "msg_apv_autoChkApproval", "msg_apv_autoChkAssist"
                   ...
                   , "lbl_apv_comment", "lbl_apv_tit_consent", "lbl_apv_reqdept", "lbl_apv_Propdept", "lbl_apv_Acceptdept", "lbl_apv_approver"
                   ...
                   , "lbl_apv_hold", "lbl_apv_rejectedto", "lbl_preview", "lbl_apv_AddFile", "lbl_download_all", "lbl_apv_sign_approval"
                   , "msg_apv_057", "msg_apv_058", "msg_apv_059", "msg_apv_173", "btn_apv_RedraftWithdraw", "msg_apv_holdForward", "msg_apv_holdOther"
				   , "msg_ApvNoReserveDraftDate", "msg_ApvNoReserveDistDate", "msg_ApvReserveDraftDate_01", "msg_ApvReserveDraftDate_02"
				   , "msg_apv_357", "msg_apv_358", "msg_apv_359", "msg_apv_360", "msg_apv_361", "msg_apv_362", "lbl_apv_cclisttitle", "msg_apv_setFormRuleErrorMsg", "msg_apv_051", "msg_apv_052"
				   , "lbl_apv_apvType", "btn_apv_consultors", "btn_apv_consultors_2", "lbl_apv_assist", "lbl_apv_assist_2", "lbl_apv_DeptConsent", "lbl_apv_DeptConsent_2", "lbl_apv_DeptAssist", "lbl_apv_DeptAssist2", "lbl_apv_DeptAssist_2"
				   , "msg_apv_recInputApvPW", "lbl_apv_ApvPwd_Check", "msg_PasswordChange_02", "lbl_RequestUserAuth", "lbl_Biometrics"]);
...
Common.getBaseConfigList(["CheckApprovalEditorLoad", "EditorType", "MobileDocConverterURL", "RETURN_FMPF", "useHold", "VacationForm", "ApprovalLineMax_Table"
                          			, "ServerDateSimpleFormat", "CalenderDateFormat", "CalenderDateFormat", "DateFullFormat", "useWebhardAttach", "EditorMinHeight"
                          			, "MailUse"]);
...
function postEditorJob(elm) {
    ...
    if (getInfo("Request.templatemode") == "Read" && getInfo("FormInfo.FormPrefix") == "WF_FORM_EXTERNAL") {
    	...
    	var minHeight = coviCmn.configMap.EditorMinHeight == "" ? "580" : coviCmn.configMap.EditorMinHeight;
...
function EDMSDocLink() {
	...
	window._CallBackMethod = function(linkDocArr){
		callBackAddEdmsDocLink(linkDocArr);
	};
	CFN_OpenWindow(sUrl, "", 850, 660, "fix");
}

function callBackAddEdmsDocLink(pLinkDocArr){
	
	var newLinkDocArr = new Array();
	var strNewLinkDocArr = "";
	var strOldLinkDocs = $("#EDMSDocLinks").val();
	if(strOldLinkDocs != ""){
		newLinkDocArr = strOldLinkDocs.split("^^^");
	}

	$.each(pLinkDocArr, function(nIdx, newItem){
		var docLinkInfo = newItem.BizSection + "@@@" + newItem.MenuID + "@@@" + newItem.Version + "@@@" + newItem.FolderID; 
		docLinkInfo += "@@@" + newItem.MessageID + "@@@" + newItem.DisplayName;
		newLinkDocArr.push(docLinkInfo);
	});
	
	newLinkDocArr = newLinkDocArr.filter(function(item, idx){ return newLinkDocArr.indexOf(item) === idx; });
	
	$("#EDMSDocLinks").val(newLinkDocArr.join("^^^"));
...
function initOnloadformmenu_notelist() {	
    if (getInfo("Request.doclisttype") == "88") {
        ...
        if(  "SENDWAIT,ACK_REQRESEND,ACK_FAIL".indexOf( govState ) > -1 ) {
        	...
        	params.statusCd === "send" 	&& $("#btGovDocsReceipt,#btGovDocsDeny,#btGovDocsReqReSend").show();
        	params.statusCd === "accept"&& $("#btGovDocsManager").show();
