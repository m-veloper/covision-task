...
var EASY = (function () {
	...
	*/
    message = {
        ...
        empty: '{0} ' + Common.getDic('lbl_apv_noenter'), // 입력되지 않았습니다
	...
	isPeriod = function (el) {
		...
		$el.each(function () {
			...
			if ($that.closest(period_container).length > 0) {
				...
				$t = $('[name='+'"'+trigger+'"'+']', $that.closest(period_container)) //[2017-01-19] yjlee 수정
			} else {
				$s = $('#' + refArray[0]);
				$d = $('#' + refArray[1]);
				$t = $('#' + trigger);
	...
	maxChar = function (el) {
		...
		function bindEvent(el) {
			el.on('keyup change', function (event) { checkVal(el); });
		}
		...
		function getSum($this) {
			...
			if (sType == 'currency') {
                ...
                if(typeof getInfo != 'undefined') {// 관리자 - Xeasy컴포넌트에서 getInfo 속성이 없다는 오류 발생으로 예외처리함.
                    if(getInfo("FormInfo.FormPrefix") == "WF_COVI_03") {
...
//150902: 변경: 부분합(열 소계) 기능 추가
	...
	function watchValue(obj) {
		...
		$(elems).each(function () {
			var target = this;
			//var source = $(this).data().watch;		[gypark 2017-05-23] firefox에서 .data() 가 제대로 동작하지 않음
			var source = typeof $(this).data().watch == "string" ? $(this).data().watch : $(this).attr("data-watch"); 
			//watch list
	...
	var listenDomAddEvent = function () {
		...
		$('body').on('DOMNodeInserted,DOMSubtreeModified', function (event) {
	...
	function checkMode() {
		...
		if (typeof getInfo === 'function') {
			m = getInfo("Request.templatemode"); //Read or Write
