		...
		if (useRightTableOption) {
				...
				if (i == 0 && appLength <= tableLineMax) {
					...
					if (i == 1) {  // 2번째 division
						if (bRightTableLineExist) {
							if (step_count > 0) {
								...
								__rightapplinetable = __initApvListCols(mobile_comm_getDic("btn_apv_managedept"), mobile_comm_getDicInfo(sOUName));
							}
						} else {
						...
						if (bRightTableLineExist) {
							...
							if (step_count > 0) {
								...
								__rightapplinetable = __initApvListCols(mobile_comm_getDic("initApvListCols"), mobile_comm_getDicInfo(sOUName));
							}
						} else {
				   ...
				   if (division_length - 1 < aOUName.length && division_length - 1 == i) {
						...
						for (var kk = i - noDisplay; kk < aOUName.length; kk++) {                               
							__rightapplinetable =  __initApvListCols( mobile_comm_getDic("btn_apv_managedept"), mobile_comm_getDicInfo(aOUName[kk]) );
			...
			//--------------------------------------------------------------------/
				...
				for (var kk = 0; kk < aOUName.length; kk++) {
					...
					if (appLength > tableLineMax) {                                
						var l_Dic = mobile_comm_getDicAll('lbl_apv_Approved;lbl_apv_username;lbl_dept;lbl_sign;lbl_apv_date;lbl_ApprovalState');
						__applinelisttype.tableHeader['apvKind'] = l_Dic['lbl_apv_Approved'];
						__applinelisttype.tableHeader['name'] = l_Dic['lbl_apv_username'];
						__applinelisttype.tableHeader['dept'] = l_Dic['lbl_dept'];
						__applinelisttype.tableHeader['signImage'] = l_Dic['lbl_sign'] ;
						__applinelisttype.tableHeader['date'] = l_Dic['lbl_apv_date'];
						__applinelisttype.tableHeader['state'] = l_Dic['lbl_ApprovalState'];
						...
						__linedata['dept'] = mobile_comm_getDicInfo(aOUName[kk]);
    ...
    if ($$_elmRoot.exist()) {
        ...
        $$_elmRoot.find("division").concat().each(function (i, $$_element) {
            ...
            /*if (document.getElementById("displayApvList") != null) {
                ...
                __tdo.tables.AssistLine = __initApvListCols(mobile_comm_getDic("lbl_apv_tit_consent"), "/");	//m_oFormMenu.gLabel_tit_consent, "/");
            }*/
            ...
            if ( _sc('scPRec') == 'N' && _sc('scDRec') == 'N' && _sc('scIPub') == 'N')//일반결재
            {
				...
				__tabledata.tableHeader.apvKind = l_Dic['lbl_apv_Approved']; 				//결재유형 - 담당, 결재, 합의, 감사...   
				__tabledata.tableHeader.name = l_Dic['lbl_apv_username']; 					//이름
				__tabledata.tableHeader.dept = l_Dic['lbl_dept']; 							//부서
				__tabledata.tableHeader.signImage = l_Dic['lbl_sign']; 						//서명이미지 - 서명 이미지경로
				__tabledata.tableHeader.state = l_Dic['lbl_ApprovalState']; 				//결재상태 - 승인, 반려...          
    ...
    for (var iStep = 0; iStep < $$_steps.length; iStep++) {
        ...
        if ($$_step.attr("unittype") == "ou") {
            ...
            for (var iOu = 0; iOu < $$_ous.length; iOu++) {
                ...
                else if ($$_stepou.parent().attr("routetype") == "audit") { Apvdecide = mobile_comm_getDic("lbl_apv_audit"); }			//"감사"
                else if ($$_stepou.parent().attr("routetype") == "audit" && $$_stepou.parent().attr("name") == "audit_dept") { Apvdecide = mobile_comm_getDic("lbl_apv_dept_audit"); } //부서준법
...
function __displayAssist($$_elmRoot) {
	...
	$$_persons.each(function (index, $$_person) {
		...
		if ($$_person.nodename() == "role") {
			...
			try { sTitle = mobile_comm_getDicInfo($$_person.attr("name")); } catch (e) { }
		} else {
			sCode = $$_person.attr("code");
			sTitle = mobile_comm_getDicInfo($$_person.attr("name")) + " " + getLngLabel($$_person.attr("position"), true);
		}
		...
		if ($$_ou.find("taskinfo").eq(0).attr("visible") != "n") {
			...
			$$_ou.find("person").has(">taskinfo[visible!='n']").concat().each(function (p, $$_person) {
				...
				if ($$_person.find("taskinfo").attr("kind") == "substitute") {
					 person_substitute = mobile_comm_getDicInfo($$_person.attr("name")) + " " + getLngLabel($$_person.attr("position"), true) + "<b>(" + mobile_comm_getDic("lbl_apv_substitue") + ")</b><br/>";
                     return;
				}
    ...
    if (sDisplayType != null && sDisplayType == "list") {
        if (sDate != "") {
            if (signtype != "" && signtype != null && signtype != "undefined") {
                 rtn = signtype == "stamp" ? mobile_comm_getDicInfo(apvname) :  __basedomain + g_BaseImgURL + signtype;
            } else {
                ...
                if ($$_elmRoot.find("division").valLength() > 1) {
                    if (i == 0) {
                        ...
                        if (appLength > tableLineMax) {
                            ...
                            try { document.getElementById("LApvLine").innerHTML = getRequestApvList($$_elmList, $$_elmVisible, "", false, mobile_comm_getDic("btn_apv_approval2")).replace("table_1", "table_1_left"); } catch (e) { document.getElementById("AppLine").innerHTML = getRequestApvList($$_elmList, "", true, mobile_comm_getDic("btn_apv_approval2")); }
                        }
                    }
                        ...
                        if (i == 1) {
                            try {
                                if (document.getElementById("RApvLine" + i) != null) {
                                    document.getElementById("RApvLine" + i).innerHTML = getRequestApvList($$_elmList, $$_elmVisible, "", true, mobile_comm_getDic("btn_apv_managedept"), true);
                                    ...
                                    if (document.getElementById("RApvLine") != null) {
                                        if ($$_elmList.valLength() > 0) {
                                            document.getElementById("RApvLine").innerHTML = getRequestApvList($$_elmList, $$_elmVisible, "", true, mobile_comm_getDic("btn_apv_managedept"), true);
                                        } else {
                                            document.getElementById("RApvLine").innerHTML = initApvListCols(mobile_comm_getDic("btn_apv_managedept"), mobile_comm_getDicInfo(sOUName));	//m_oFormMenu.gLabel_managedept, mobile_comm_getDicInfo(sOUName));
                                        }
                                    } else {
                            ...
                            try {
                                if (document.getElementById("RApvLine" + i) != null) {
                                    ...
                                    if (document.getElementById("RApvLine") != null) {
                                        ...
                                        //n단 결재시(n개 수신부서) 기안시점에 결재방에 그려지지 않아서
                                        //document.getElementById("RApvLine").innerHTML = szPreRApvLine + ((szPreRApvLine == "") ? "" : "<p style='margin:0px; padding:1px;' />") + getRequestApvList(elmList, elmVisible, "", true, m_oFormMenu.gLabel_managedept, true);
                                        if ($$_elmList.valLength() > 0) {
                                            document.getElementById("RApvLine").innerHTML = szPreRApvLine + ((szPreRApvLine == "") ? "" : "<p style='margin:0px; padding:1px;' />") + getRequestApvList($$_elmList, $$_elmVisible, "", true, mobile_comm_getDic("btn_apv_managedept"), true);
                                        } else {
                                            document.getElementById("RApvLine").innerHTML = szPreRApvLine + ((szPreRApvLine == "") ? "" : "<p style='margin:0px; padding:1px;' />") + initApvListCols(mobile_comm_getDic("initApvListCols"), mobile_comm_getDicInfo(sOUName));		 //m_oFormMenu.gLabel_managedept, mobile_comm_getDicInfo(sOUName));
                                        }

                                    } else {
                                        document.getElementById("AppLine").innerHTML = getRequestApvList($$_elmList, "", true, mobile_comm_getDicInfo($(elm).attr("ouname"))); //JAL-??? elm not defined
                                    }
                                }
                           } catch (e) {  }
                       }
                       ...
                       if ($$_elmRoot.find("division").valLength() - 1 < aOUName.length && $$_elmRoot.find("division").valLength() - 1 == i) {
                            ...
                            for (var kk = i - noDisplay; kk < aOUName.length; kk++) {
                                if (document.getElementById("RApvLine" + (kk + 1)) != null) {
                                    document.getElementById("RApvLine" + (kk + 1)).innerHTML = initApvListCols(mobile_comm_getDic("btn_apv_managedept"), mobile_comm_getDicInfo(aOUName[kk]));	//m_oFormMenu.gLabel_managedept, mobile_comm_getDicInfo(aOUName[kk]));
                                } else {
                                    document.getElementById("RApvLine").innerHTML += "<p style='margin:0px; padding:1px;' />" + initApvListCols(mobile_comm_getDic("btn_apv_managedept"), mobile_comm_getDicInfo(aOUName[kk]));		//m_oFormMenu.gLabel_managedept, mobile_comm_getDicInfo(aOUName[kk]));
                                }
                            }
                        }
                    }
                    ...
                    if (i == 0) {
                        ...
                        if (appLength > tableLineMax) {
                            ...
                            try { document.getElementById("LApvLine").innerHTML = getRequestApvList($$_elmList, $$_elmVisible, "", false, mobile_comm_getDic("btn_apv_approval2")); } catch (e) {
                                document.getElementById("AppLine").innerHTML = getRequestApvList($$_elmList, "", true, mobile_comm_getDicInfo($(elm).attr("ouname")));
                            }
                        }
                        //담당부서/담당업무뿌려주기
                        for (var kk = 0; kk < aOUName.length; kk++) {
                            ...
                            if (appLength > tableLineMax) {                                
                                ...
                                document.getElementById("AppLineListType").innerHTML = document.getElementById("AppLineListType").innerHTML                                    
                                    ...
                                    + "<tr>"
                                    + "<td align='center' valign='middle' style='border-bottom:1px #bcbcbc solid; height:25px; line-height:25px;'><strong>" + l_Dic["lbl_apv_Approved"] + "</strong></td>"
                                    + "<td align='center' style='border-bottom:1px #bcbcbc solid;'><strong>" + l_Dic["lbl_apv_username"] + "</strong></td>"
                                    + "<td align='center' style='border-bottom:1px #bcbcbc solid;'><strong>" + l_Dic["lbl_dept"] + "</strong></td>"
                                    + "<td align='center' style='border-bottom:1px #bcbcbc solid;'><strong>" + l_Dic["lbl_sign"] + "</strong></td>"
                                    + "<td align='center' style='border-bottom:1px #bcbcbc solid;'><strong>" + l_Dic["lbl_apv_date"] + "</strong></td>"
                                    + "<td align='center' style='border-bottom:1px #bcbcbc solid;'><strong>" + l_Dic["lbl_ApprovalState"] + "</strong></td>"
                                    ...
                                    + "<td align='center'>&nbsp;</td>"
                                    + "<td align='center'>" + mobile_comm_getDicInfo(aOUName[kk]) + "</td>"
                                    + "<td align='center'>&nbsp;</td>"
                                    + "<td align='center'>&nbsp;</td>"
                                    + "<td align='center'>&nbsp;</td>"
                                    + "<td align='center'>&nbsp;</td>"
                                ...
                                if (document.getElementById("RApvLine" + (kk + 1)) != null) {
                                    ...
                                    document.getElementById("RApvLine").innerHTML = ((kk == 0) ? "" : document.getElementById("RApvLine").innerHTML + "<p style='margin:0px; padding:1px;' />") + initApvListCols(mobile_comm_getDic("btn_apv_managedept"), mobile_comm_getDicInfo(aOUName[kk]));		//m_oFormMenu.gLabel_managedept, mobile_comm_getDicInfo(aOUName[kk]));
                                }
                            }
                        }
                    }
                ...
                if ($$_elmRoot.find("division").valLength() > 1) {
                    if (i == 0) {
                        ...
                        if (appLength > tableLineMax) {
                            ...
                            try { 
                                document.getElementById("LApvLine").innerHTML = getRequestApvList($$_elmList, $$_elmVisible, "", false, mobile_comm_getDic("btn_apv_approval2")); 
                                
                            } catch (e) { 
                                document.getElementById("AppLine").innerHTML = getRequestApvList($$_elmList, "", true, mobile_comm_getDicInfo($$_element.attr("ouname"))); 
                    ...
                    if (i == 1) {
                    	...
                    	try { 
                    	    document.getElementById("RApvLine").innerHTML = getRequestApvList($$_elmList, $$_elmVisible, "", true, mobile_comm_getDic("btn_apv_managedept")); 
                    	} catch (e) { 
                    	    document.getElementById("AppLine").innerHTML = getRequestApvList($$_elmList, "", true, m_mobile_comm_getDic("btn_apv_managedept")); 
                    	}

                    	//배포일때는 결재방에 수신처로 표시되도록
                    	if (getInfo("SchemaContext.scIPub.isUse") == "Y" || getInfo("SchemaContext.scGRec.isUse") == "Y") {
                    		try { 
                    		    document.getElementById("RApvLine").innerHTML = getRequestApvList($$_elmList, $$_elmVisible, "", true, mobile_comm_getDic("btn_apv_recvedept"));
                    		} catch (e) { 
                    		    document.getElementById("AppLine").innerHTML = getRequestApvList($$_elmList, "", true, mobile_comm_getDic("btn_apv_recvedept")); 
                    		}
                    	}
                    }
                } else {
                    ...
                    if (appLength > tableLineMax) {
                        ...
                        document.getElementById("AppLine").innerHTML = getRequestApvList($$_elmList, $$_elmVisible, "", false, mobile_comm_getDic("btn_apv_approval2"));
                    }
                }
            }
        }
...
function getRequestApvList($$_elmList, $$_elmVisible, sMode, bReceive, sApvTitle, bDisplayCharge) {//신청서html
    ...
    $$_elmList.concat().each(function (i, $$_elm) {
        ...
        if ($$_elmTaskInfo.attr("kind") != 'conveyance' && $$_elmTaskInfo.attr("kind") != 'bypass') {
            ...
            if (i == 0) {
                if (bReceive) { //담당부서 결재선
                    if (sApvTitle != "") { ApvPOSLines += "<th class='tit'>" + (sApvTitle == undefined ? mobile_comm_getDic("lbl_apv_management") + "<br>" + mobile_comm_getDic("lbl_apv_dept") : sApvTitle) + "</th>"; }
                } else {
                    if (sApvTitle != "") { ApvPOSLines += "<th class='tit'>" + (sApvTitle == undefined ? mobile_comm_getDic("lbl_apv_request2") + "<br>" + mobile_comm_getDic("lbl_apv_dept") : sApvTitle) + "</th>"; }
                }
            }
            if ((bDisplayCharge && i == 0) || ($$_elmTaskInfo.attr("visible") != "n")) //결재선 숨기기한 사람 숨기기
            {
                ...
                if (strDate == null && $$_elmTaskInfo.attr("result") != "reserved") {   //보류도 결재방에 표시
                    ...
                    // 수신,발신처 있을경우의 문서 이관시 '의견' 란 링크 삭제
                    if (m_oFormMenu.m_CmtBln == false) { ApvCmt += (!$$_assistcmt.exist()) ? "&nbsp;" : mobile_comm_getDic("gLabel_comment"); }		//m_oFormMenu.gLabel_comment; }
                    else
                    { ApvCmt += (!$$_assistcmt.exist()) ? "&nbsp;" : "<a href=\'#\' onclick=\'viewComment(\"" + i + "\")\'>" + mobile_comm_getDic("gLabel_comment") + "</a>"; }
                }
                ...
                switch ($$_elmTaskInfo.attr("kind")) {
                    case "authorize":
                        ApvSignLines += (strDate == "") ? mobile_comm_getDicInfo($$_elm.attr("name")) + "<br /><font size='2'>(" + mobile_comm_getDic("lbl_apv_authorize") + ")</font>" : ((bRejected) ? interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind")) : getSignUrl(sCode, $$_elmTaskInfo.attr("customattribute1"), $$_elm.attr("name"), strDate, true, $$_elmTaskInfo.attr("result"), true));
                        ApvApproveNameLines += (strDate == "") ? "<span class='txt_gn11_blur'>&nbsp;</span>" : "<span class='txt_gn11_blur'>" + mobile_comm_getDicInfo($$_elm.attr("name")) + " " + mobile_comm_getDic("lbl_apv_authorize") + "</span>";
                        ApvDateLines += (strDate == "") ? "&nbsp;" : formatDate(strDate, "H");
                        LastApv = "/";
                        LastApvName = mobile_comm_getDicInfo($($$_elmname).attr("name")) + interpretResult($$_elmTaskInfo.attr("result")) + "<br />";
                        LastDate = formatDate(strDate);
                    ...
                    case "substitute":
                    	//원결재자 표시가 안되서 수정
                        ApvSignLines += (strDate == "") ? mobile_comm_getDicInfo($$_elm.attr("name")) + "<br /><font size='2'>(" + mobile_comm_getDic("lbl_apv_substitue") + ")</font>" : ((bRejected) ? interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind")) : "<font size='2'>(원) " + mobile_comm_getDicInfo($$_elmList.concat().eq(i+1).attr("name")) + "<br />" + mobile_comm_getDicInfo($$_elm.attr("name")) + "</font>");
                        ApvApproveNameLines += (strDate == "") ? "<span class='txt_gn11_blur'>&nbsp;</span>" : "<span class='txt_gn11_blur'>" + mobile_comm_getDicInfo($$_elm.attr("name")) + " " + mobile_comm_getDic("lbl_apv_substitue") + "</span>";
                        ApvDateLines += (strDate == "") ? "&nbsp;" : formatDate(strDate, "H");
                        LastApv = "";
                        LastApvName = "";
                        LastDate = "";
                    ...
                    case "skip":
                        ...
                        break;
                    case "bypass":
                        ApvSignLines += (strDate == "") ? mobile_comm_getDicInfo($$_elm.attr("name")) + "<br /><font size='2'>(" + mobile_comm_getDic("lbl_apv_bypass") + ")</font>" : getSignUrl(sCode, $$_elmTaskInfo.attr("customattribute1"), $$_elm.attr("name"), strDate, true, $$_elmTaskInfo.attr("result"), true);
                        ApvApproveNameLines += (strDate == "") ? "<span class='txt_gn11_blur'>&nbsp;</span>" : "<span class='txt_gn11_blur'>" + mobile_comm_getDicInfo($$_elm.attr("name")) + " " + mobile_comm_getDic("lbl_apv_bypass") + "</span>";
                        ApvDateLines += (strDate == "") ? "&nbsp;" : formatDate(strDate, "H");
                        break; //"후열"
                    case "review":
                        ApvSignLines += (strDate == "") ? mobile_comm_getDicInfo($$_elm.attr("name")) + "<br /><font size='2'>(" + mobile_comm_getDic("lbl_apv_review") + ")</font>" : getSignUrl(sCode, $$_elmTaskInfo.attr("customattribute1"), $$_elm.attr("name"), strDate, true, $$_elmTaskInfo.attr("result"), true);
                        ApvApproveNameLines += (strDate == "") ? "<span class='txt_gn11_blur'>&nbsp;</span>" : "<span class='txt_gn11_blur'>" + mobile_comm_getDicInfo($$_elm.attr("name")) + " " + mobile_comm_getDic("lbl_apv_review") + "</span>";
                        ApvDateLines += (strDate == "") ? "&nbsp;" : formatDate(strDate, "H");
                        break;
                    case "charge":
                        //담당 수신자 대결 S ---------------------------------------------
                        ...
                        if (isDuptyCheck) {     //대결 표시
                            //원결재자 표시가 안되서 수정
                            ApvSignLines += (strDate == "") ? mobile_comm_getDicInfo($$_elm.attr("name")) + "<br /><font size='2'>(" + mobile_comm_getDic("lbl_apv_substitue") + ")</font>" : ((bRejected) ? interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind")) : "<font size='2'>(원) " + mobile_comm_getDicInfo($($$_elmList).get(i + 1).attr("name")) + "<br />" + mobile_comm_getDicInfo($$_elm.attr("name")) + "</font>");
                            ApvApproveNameLines += (strDate == "") ? "<span class='txt_gn11_blur'>&nbsp;</span>" : "<span class='txt_gn11_blur'>" + mobile_comm_getDicInfo($$_elm.attr("name")) + " " + mobile_comm_getDic("lbl_apv_substitue") + "</span>";
                            ApvDateLines += (strDate == "") ? "&nbsp;" : formatDate(strDate, "H");
                            LastApv = "";
                            LastApvName = "";
                            ...
                            ApvSignLines += (strDate == "") ? mobile_comm_getDicInfo($$_elm.attr("name")) : ((bRejected) ? interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind")) : getSignUrl(sCode, (($$_elmTaskInfo.attr("customattribute1") == undefined) ? "stamp" : $$_elmTaskInfo.attr("customattribute1")), $$_elm.attr("name"), strDate, true, $$_elmTaskInfo.attr("result"), true));
                            ApvApproveNameLines += (strDate == "") ? "<span class='txt_gn11_blur'>&nbsp;</span>" : "<span class='txt_gn11_blur'>" + mobile_comm_getDicInfo($$_elm.attr("name")) + "</span>";
                            ApvDateLines += (strDate == "") ? "&nbsp;" : formatDate(strDate, "H");
                        }
                        break;
                    default:
                        ApvSignLines += (strDate == "") ? mobile_comm_getDicInfo($$_elm.attr("name")) : ((bRejected) ? interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind")) : getSignUrl(sCode, $$_elmTaskInfo.attr("customattribute1"), $$_elm.attr("name"), strDate, true, $$_elmTaskInfo.attr("result"), true));
                        ApvApproveNameLines += (strDate == "") ? "<span class='txt_gn11_blur'>&nbsp;</span>" : "<span class='txt_gn11_blur'>" + mobile_comm_getDicInfo($$_elm.attr("name")) + ((bRejected) ? "&nbsp;" : " " + interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind"))) + "</span>";
                        ApvDateLines += (strDate == "") ? "&nbsp;" : formatDate(strDate, "H") + "&nbsp;";
                }
                ApvSignLines += "</td></tr>";
...
function displayAssist($$_elmRoot) {
    ...
    if ($$_elmListCount != 0) {
        Apvlines = "<tr><th style>" + mobile_comm_getDic("lbl_apv_dept") + "</th>";
        Apvlines += "	<th>" + mobile_comm_getDic("lbl_apv_username") + "</th>";
        Apvlines += "	<th>" + mobile_comm_getDic("lbl_apv_approvdate") + "</th>";
        Apvlines += "   <th>" + mobile_comm_getDic("lbl_apv_comment") + "</th></tr>";
        $$_elmOPList.concat().each(function (index, $$_element) {
            var sCode = "";
            var sTitle = "";
            if ($$_element.nodeName == "role") {
                try { sCode = $$_element.find("person").attr("code"); } catch (e) { }
                try { sTitle = mobile_comm_getDicInfo($$_element.attr("name")); } catch (e) { }
            } else {
                sCode = $$_element.attr("code");
                sTitle = mobile_comm_getDicInfo($$_element.attr("name")) + " " + getLngLabel($$_element.attr("position"), true);
            }
            ...
            if ($$_elmTaskInfo.attr("visible") != "n") {
                ...
                switch ($$_elmTaskInfo.attr("kind")) {
                    ...
                    case "substitute":
                        LastTitle = getPresence(sCode, "assist" + i + sCode, $$_element.attr("sipaddress")) + mobile_comm_getDicInfo($$_element.attr("name"));
                    ...
                    case "bypass":
                        Apvlines += "<tr>";
                        Apvlines += "<td>" + mobile_comm_getDicInfo($$_element.attr("ouname")) + "</td>"; //문서창 상단에서 협조자의 의견이 잘리지 않도록 수정함.
                        Apvlines += "<td>" + sTitle + " " + mobile_comm_getDic("lbl_apv_substitue") + " " + LastTitle + "</td>";
        ...
        $$_elmOUList.each(function (index, $$_element) {
            ...
            if (tmp_rowspan == 0) {
                ...
                Apvlines += "<tr>";
                Apvlines += "<td rowspan='" + tmp_rowspan + "'>" + mobile_comm_getDicInfo($$_element.attr("name")) + "</td>";
            }
            ...
            if ($$_elmTaskInfo.attr("visible") != "n") {
                $$_element.find("person:has(taskinfo[visible!='n'])").each(function (p, person) {
                    ...
                    if ($$_person.find("taskinfo").attr("kind") == "review") {    //부서합의 후결일 경우
                        Apvlines += "<td>" + mobile_comm_getDicInfo($$_person.attr("name")) + "&nbsp;<b>(후결)<b>" + "</td>";
                    } else {
                        Apvlines += "<td>" + mobile_comm_getDicInfo($$_person.attr("name")) + "</td>";
                    }
                    Apvlines += "<td>" + ((strDate == "") ? "&nbsp;" : formatDate(strDate) + " " + interpretResult($$_person.find("taskinfo").attr("result"))) + "</td>";
...
function displayCCInfo($$_elmRoot) {
    ...
    if ($$_ccInfos.length > 0) {
        $$_ccInfos.concat().each(function (index, $$_ccInfo) {
            ...
            while ($$_cc.exist() && ccListIndex < cc_maxlimit) {
                ...
                if ($$_cc.nodename() == "person") {
                    sList += (sList.length > 0 ? ";" : "") + mobile_comm_getDicInfo($$_cc.attr("ouname")) + " " + getLngLabel($$_cc.attr("title"), true) + " " + mobile_comm_getDicInfo($$_cc.attr("name"));
        ...
        //참조자 목록 보이기 처리
        try { $("#CCLine").show(); } catch (e) { }
    } else {
        try { $("#CCLine").hide(); } catch (e) { }
    }
}
...
function displayApvListCols(oApvList) {
    ...
    var elmRoot, elmList, elm, elmTaskInfo, elmReceive, ApvList, emlSkip, elmCmtList; //elmR : 다중수신부서 여부를 알기위한 nodes 변수
    var Apvlines = "";
    var strDate, strFieldName, strwidth, strColTD, LastDate;
    var rtnPOSLine, stepList, TaskInfo, writeTaskInfo, rtnConsentLine, ConsentLine;
    var l_Dic = mobile_comm_getDicAll('lbl_apv_Approved;lbl_apv_username;lbl_dept;lbl_sign;lbl_apv_date;lbl_ApprovalState');
    ...
    if ($$_elmRoot.exist()) {
        ...
        $$_elmRoot.find("division").concat().each(function (i, $$_element) {
            ...
            if (document.getElementById("displayApvList") != null) {
                ...
                document.getElementById("AppLine").innerHTML = initApvListCols(mobile_comm_getDic("btn_apv_approval2"), "/");
                document.getElementById("AssistLine").innerHTML = initApvListCols(mobile_comm_getDic("lbl_apv_tit_consent"), "/");
            }
    ...
    for (var ii = 0; ii < $$_elmListSteps.length; ii++) {
        ...
        if ($$_elmStep.attr("unittype") == "ou") {
            ...
            for (var ij = 0; ij < $$_elmList.length; ij++) {
                ...
                if ($$_elm.parent().attr("routetype") == "consult") { Apvdecide = mobile_comm_getDic("lbl_apv_DeptConsent"); } //"부 서 합 의"
                if ($$_elm.parent().attr("routetype") == "assist") { Apvdecide = mobile_comm_getDic("lbl_apv_DeptAssist"); } //"개 인 합 의"
                if ($$_elm.parent().attr("routetype") == "audit") { Apvdecide = mobile_comm_getDic("lbl_apv_audit"); } //"감사"
                if ($$_elm.parent().attr("routetype") == "audit" && $$_elm.parent().attr("name") == "audit_dept") { Apvdecide = mobile_comm_getDic("lbl_apv_dept_audit"); }
                if ($$_elm.parent().attr("name") == "ExtType") { Apvdecide = mobile_comm_getDic("lbl_apv_ExtType"); } //"심 의" -> 특이기능
                ApvSignLines += (strDate == "") ? "" : getSignUrl(sCode, $$_elmTaskInfo.attr("customattribute1"), "", strDate, false, $$_elmTaskInfo.attr("result"), false);
                ApvState = interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind"));
                ApvDept = mobile_comm_getDicInfo($$_elm.attr("name"));
                ApvDateLines = "";
                ApvDateLines += (strDate == "") ? "&nbsp;" : formatDate(strDate);
                ApvApproveNameLines += "&nbsp;&nbsp;&nbsp;&nbsp;" + ApvDateLines;
                        ...
                        if ($$_elmTaskInfo.attr("kind") != 'skip') {
                            ...
                            switch ($$_elmTaskInfo.attr("kind")) {
                                case "authorize":
                                    ...
                                    ApvApproveNameLines += "&nbsp;&nbsp;&nbsp;&nbsp;" + ApvDateLines;
                                    LastApv = "/";
                                    LastApvName = mobile_comm_getDicInfo($$_elm.attr("name")) + interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind"));
                                    LastDate = formatDate(strDate, "APV");
                                    //ApvKind = "전결";
                                    break;
                                case "substitute":
                                    ApvSignLines += mobile_comm_getDicInfo($$_elm.attr("name"));
                                    ApvSignLines += "<br />(" + mobile_comm_getDic("lbl_apv_substitue") + ")";		//m_oFormMenu.gLabel_substitue + ")";
                                    ApvDept = mobile_comm_getDicInfo($$_elm.attr("ouname"));
                                    ApvSign = (strDate == "") ? "&nbsp;" : getSignUrl(sCode, $$_elmTaskInfo.attr("customattribute1"), $$_elm.attr("name"), strDate, false, $$_elmTaskInfo.attr("result"), true, "list");
                                    ApvState = interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind"));
                                    ApvDateLines = "";
                                    ...
                                    //                                OriginApprover = mobile_comm_getDicInfo(nextelm.attr("name"));
                                    //ApvKind = "대결";
                                ...
                                case "bypass":
                                    ...
                                    ApvState = interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind"));
                                    ApvDateLines = "";
                                    ApvDateLines += (LastDate == "") ? mobile_comm_getDic("lbl_apv_bypass") : LastDate;  //"후열"
                                    ApvApproveNameLines += (LastApvName == "") ? mobile_comm_getDic("lbl_apv_bypass") : LastApvName; //"후열"
                                    ApvApproveNameLines += "&nbsp;&nbsp;&nbsp;&nbsp;" + ApvDateLines;
                                    break; //"후열"
                                case "review":
                                    ApvSignLines += mobile_comm_getDicInfo($$_elm.attr("name"));
...
function formatDate(sDate, sMode) {
    ...
    if(Common.getBaseConfig("BaseTimeZone_ApvLine") != "" && Common.getBaseConfig("BaseTimeZone_ApvLineTo") != "" && sDate > Common.getBaseConfig("BaseTimeZone_ApvLine") && sDate < Common.getBaseConfig("BaseTimeZone_ApvLineTo")){
    	...
    	dtDate = XFN_TransDateServerFormat(formatDateCVT(sDate));
    }
	    ...
	    if(elm.parent().attr("datecompleted") == null) {
		    ...
		    if(elm.parent().find("comment_fileinfo").length > 0) {
		    	...
		    	$(Comment.comment_fileinfo).each(function (i, obj) {
			    	...
			    	var className = mobile_comm_getFileExtension(obj.name);
					...
					fileInfoHtml += "					<span class='ico_file " + className + "'>파일첨부</span>" + obj.name;
					fileInfoHtml += "				</a></dt>";
					
					if (mobile_comm_getQueryString("listpreview") != "Y") {
						fileInfoHtml += "<dd>";
						fileInfoHtml += "<a class='previewBtn fRight' href='#ax' onclick='attachFilePreview(\"" + obj.id + "\",\"" + obj.FileToken + "\",\"" + fileExt + "\");'>" + mobile_comm_getDic("lbl_preview") + "</a>";
						fileInfoHtml += "</dd>";
					}
					fileInfoHtml += "</dl>";
		    	});
					...
					$$(usedItem).concat().each(function(j, strItem){
						...
						if(strItem.json() != "apvKind"){
							if(strItem.json() == "comment" && innerApvListObjVal != "&nbsp;"){
								...
								// 기 결재자 클릭 시 해당 사용자의 프로필 조회
								if(lineDataObjVal != "&nbsp;" && $$(innerApvListObj).attr("code") != "") {
