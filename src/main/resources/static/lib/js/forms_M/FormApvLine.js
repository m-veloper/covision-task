				...
				switch ($$_elmTaskInfo.attr("kind")) {
                    ...
                    case "charge":
                        ...
                        if (isDuptyCheck) {     //대결 표시
                            //원결재자 표시가 안되서 수정
                            ApvSignLines = (strDate == "") ? CFN_GetDicInfo($$_elm.attr("name")) + "<br />(" + coviDic.dicMap.lbl_apv_substitue + ")" : ((bRejected) ? interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind")) : "(원) " + CFN_GetDicInfo($($$_elmList).get(i + 1).attr("name")) + "<br />" + CFN_GetDicInfo($$_elm.attr("name")));
                            ApvApproveNameLines = (strDate == "") ? "&nbsp;" : CFN_GetDicInfo($$_elm.attr("name")) + " " + coviDic.dicMap.lbl_apv_substitue;
                            ApvDateLines = (strDate == "") ? "&nbsp;" : formatDate(strDate, "H");
                            LastApv = "";
                            LastApvName = "";
    ...
    for (var iStep = 0; iStep < $$_steps.length; iStep++) {
        ...
        //unittype이 개인
            ...
            for (var i = 0; i < $$_personsOrRoles.length; i++) {
                ...
                $$_personOrRoleTaskinfo = $$_personOrRole.find("taskinfo");
				var __linedata = __getLineDataObject();
...
function __displayAssist($$_elmRoot) {
	...
	__tabledata.tableHeader.dept = coviDic.dicMap.lbl_dept; //1번째 컬럼
	__tabledata.tableHeader.name = coviDic.dicMap.lbl_apv_username; //2번째 컬럼
	__tabledata.tableHeader.date = coviDic.dicMap.lbl_apv_approvdate; //3번째 컬럼
	__tabledata.tableHeader.comment = coviDic.dicMap.lbl_apv_comment; //4번째 컬럼
...
function interpretResult(strresult, szExtType, szkind) {
    ...
    if (szExtType == "ExtType") {
        ...
        if (strresult == "completed") { sKind = coviDic.dicMap.lbl_apv_Approved;		/*gLabel_apv;*/ } 		//"승인"
        if (strresult == "reviewed") { sKind = coviDic.dicMap.lbl_apv_Approved;		/*gLabel_apv;*/ } 		//"승인"
        if (strresult == "rejected") { sKind = coviDic.dicMap.lbl_apv_reject;		 } 	//"반려"
