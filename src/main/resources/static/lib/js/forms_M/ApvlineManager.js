        ...
        switch (sRT) {
            ...
            case "consult":
                switch (sUT) {
                    case "ou":
                        switch (sParentUT) {
                            case "ou": sSignType = (stepallottype=="parallel")? coviDic.dicMap["lbl_apv_deptconsent_parallel"]:coviDic.dicMap["lbl_apv_DeptConsent"];  break;
                            case "role":
                            case "person": sSignType = convertKindToSignType(sKind, scustomattribute2); break;
                        } break;
                    case "role":
            ...
            case "assist":
                switch (sUT) {
                    case "ou":
                        switch (sParentUT) {
                            case "ou": sSignType = (stepallottype=="parallel")? coviDic.dicMap["lbl_apv_deptassist_parallel"] :coviDic.dicMap["lbl_apv_DeptAssist"];  break;
                            case "role":
                            case "person": sSignType = convertKindToSignType(sKind, scustomattribute2); break;
                        } break;
                    case "role":
...
function initialize() {
    ...
    if (m_oInfoSrc == null) {
        ...
        if (m_oInfoSrc == null) {
            if (opener.location.href.toUpperCase().indexOf("LISTDEPT") > -1) { m_oInfoSrc = parent; }
    ...
    for(var i=nrowTBindex-1; i>=0; i--){
    	if($(oTBody.rows[i]).children().eq(division_divisiontype).text() != "receive"){
	        ...
	        if (tmpIndex.length == 1) {
	            ...
	            tmpIndexDot = tmpIndex.substring(0, tmpIndex.lastIndexOf("-") + 1).split("-").join(".");
    ...
    $$_oSrcDoc.find("item").concat().each(function (i, $$_element) {
        ...
        if (m_sSelectedDivisionType == "receive" && m_sSelectedRouteType == "receive") {
            ...
            if (oTBody.lastChild != null) { //1 - 발신 없으면 말들기
                if ($(oTBody.lastChild).children().eq(dp_apv_no).text() != "1") {
                    ...
                    arrDivisionTaskinfoS.push("");
                    inserthtmlrow("tblApvLine", 1, viewtype, "True", "Division", arrDivisionS, arrDivisionTaskinfoS, arrStepS, arrStepTaskinfoS, arrOuS, arrOuTaskinfoS, arrRolePersonS, arrRolePersonTaskinfoS, arrRolePersonTaskinfoCommentS, arrRolePersonTaskinfoCommentLastS, "");
                }
            }
            ...
            inserthtmlrow("tblApvLine", nDivisionNo, viewtype, "True", "Division", arrDivisionR, arrDivisionTaskinfoR, arrStepR, arrStepTaskinfoR, arrOuR, arrOuTaskinfoR, arrRolePersonR, arrRolePersonTaskinfoR, arrRolePersonTaskinfoCommentR, arrRolePersonTaskinfoCommentLastR, "");
        ...
        if (m_sSelectedUnitType == "ou") {
            ...
            if (m_sSelectedRouteType == "consult") {
                ...
                arrRolePersonTaskinfo.push("review"); //roleperson_taskinfo_kind
            } else if (m_sSelectedStepRef == "confirm") {
        ...
        if (m_sSelectedRouteType == "ccinfo") {
            ...
            inserthtmlrow("tblApvLine", index, viewtype, "False", taskType, arrDivision, arrDivisionTaskinfo, arrStep, arrStepTaskinfo, arrOu, arrOuTaskinfo, arrRolePerson, arrRolePersonTaskinfo, arrRolePersonTaskinfoComment, arrRolePersonTaskinfoCommentLast, "");
        }
    }
	);
...
function moveUpDown(str) {
    ...
    var nTBodyRow = oTBody.childNodes;
    var nSelecter = 0;
    for (j = nTBodyRow.length - 1; j >= 0; j--) {
    ...
    if (evaluateApvList()) {
        ...
        if (admintype == "ADMIN") {//관리자 일 경우 결재선 변경 함수 호출
            ...
            if ((sMode == "REDRAFT") || (sMode == "SUBREDRAFT")) {
                m_oFormMenu.document.getElementById("btDeptDraft").style.display = "inline";
            ...
            if ((sMode == "DRAFT") || (sMode == "TEMPSAVE") || (sMode == "REDRAFT") || (sMode == "SUBREDRAFT") || (sMode == "APPROVAL") || (sMode == "PROCESS") || (sMode == "RECAPPROVAL") || (sMode == "SUBAPPROVAL")) {  //2007.10 박동현 (sMode == "RECAPPROVAL") 추가 [2016-04-21] 김지현(sMode == "SUBAPPROVAL") --> 합의부서내결재 추가
                ...
                //마지막 결재자인지 확인	
                var oPendingSteps = $$_m_oApvList.find("division>step:has(taskinfo[kind!='review'][kind!='bypass'][kind!='skip']):has(taskinfo[status='pending',status='reserved'])");
...
function doButtonAction(obj) {
    ...
    switch (obj.id) {
        case "btPersonOUALL": bSetDirty = true;
            addPersonOUALL();
            break;
        ...
        case "btPConsult": bSetDirty = true; addPConsult('serial'); break;
        case "btPConsult2": bSetDirty = true; addPConsult('parallel'); break;
        case "btDConsult": bSetDirty = true; addDConsult('serial'); break;
        case "btDConsult2": bSetDirty = true; addDConsult('parallel'); break;
        ...
        case "btCC": if (m_bCC) { bSetDirty = true; setCC("global","n"); } break;
        case "btSendCC": if (m_bCC) { bSetDirty = true; setCC("sender","n"); } break;
        case "btRecCC": if (m_bCC) { bSetDirty = true; setCC("receiver", "n"); } break;
        case "btSendCCBefor": if (m_bCC) { bSetDirty = true; setCC("sender", "y"); } break;
        case "btSendCCAfter": if (m_bCC) { bSetDirty = true; setCC("sender", "n"); } break;
        ...
        case "btExit": if (openID != "") { parent.Common.Close("btLine" + getInfo("FormInstanceInfo.FormInstID")); } else { window.close(); } break;
        case "btRecDept": addRecDept(); break;
        case "btDeleteRec": delList(); break;
        case "btPlPerson": bSetDirty = true; addparallelPerson(); break; //동시결재 추가
        case "btPAudit": case "btPAuditLeft": bSetDirty = true; addPAudit("audit"); break; //감사 추��� 2007.02 by sunny
        case "btPAudit1": case "btPAudit1Left": bSetDirty = true; addPAudit("audit_law"); break;
        case "btDAudit": case "btDAuditLeft": bSetDirty = true; addDAudit("audit_dept"); break;
        case "btDAudit1": case "btDAudit1Left": bSetDirty = true; addDAudit("audit_law_dept"); break; //준법처
        ...
        case "btDeletePersonOUALL": bSetDirty = true; deletePerson(); break;
        case "btDeleteCCALL": bSetDirty = true; deleteCC(); break;
        case "btPersonalLineSave": personalLine(); break;
        case "btDeleteRecALL": delList(); break;
        case "btAssistGroup": assistGroup(); break;
        case "btAssistGroupDelete": deleteAssistGroup(); break;
        ...
        switch (m_sApvMode) {
            ...
            default:
                ...
                if (oCheckSteps) {
                    ...
                    oTaskinfo.setAttribute("datereceived", getInfo("AppInfo.svdt"));
                    ...
                    oDivTaskinfo.setAttribute("datereceived", getInfo("AppInfo.svdt"));
...
function evaluateApvList() {
    ...
    if ((getInfo("SchemaContext.scPAgr.isUse") == "Y" || getInfo("SchemaContext.scPAgrSEQ.isUse") == "Y" )&& (m_sApvMode == "DRAFT" || m_sApvMode == "TEMPSAVE" || m_sApvMode == "APPROVAL" || m_sApvMode == "PCONSULT")) {
        var $$_elmConsult = $$_elmRoot.find("division>step[routetype='consult'][unittype='person'][allottype='parallel']");
        if (!$$_elmConsult.exist()) {
            if (getInfo("SchemaContext.scDAgr.isUse") == "Y") {
                if (!chkConsultAppLine($$_elmRoot)) return false;
            }
        } else {
            if (getInfo("SchemaContext.scACLimit.isUse") == "Y" && getInfo("SchemaContext.scACLimit.value") != '' && $$_elmConsult.find("ou").length > parseInt(getInfo("SchemaContext.scACLimit.value"))) {
    ...
    if (getInfo("SchemaContext.scDAgr.isUse") == "Y" && (m_sApvMode == "DRAFT" || m_sApvMode == "TEMPSAVE" || m_sApvMode == "APPROVAL" || m_sApvMode == "PCONSULT")) {
        var $$_elmConsult = $$_elmRoot.find("division>step[routetype='consult'][unittype='ou'][allottype='parallel']");
        if (!$$_elmConsult.exist()) {
    ...
    if (getInfo("SchemaContext.scPAgr.isUse") == 2 && (m_sApvMode == "REDRAFT" || m_sApvMode == "RECAPPROVAL")) {
        var $$_elmConsult = $$_elmRoot.find("division>step[routetype='consult'][unittype='person'][allottype='parallel']");
        if (!$$_elmConsult.exist()) {
        } else {
...
var strlable_DAsist = coviDic.dicMap["lbl_apv_DAsist"];
